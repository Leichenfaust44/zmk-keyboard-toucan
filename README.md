...
Custom ZMK firmware for the beekeeb Toucan Keyboard using the Gallium v2 Columnar Staggered layout, optimised for Swiss German (DE-CH) input.
...
![Keymap](https://github.com/Leichenfaust44/zmk-keyboard-toucan/blob/main/keymap-drawer/toucan.svg)
---
Display
The left half features a nice!view gem display (Sharp Memory LCD). It shows the following widgets from top to bottom:
![Display](https://github.com/Leichenfaust44/zmk-keyboard-toucan/blob/main/keymap-drawer/toucan_display.svg)
Widget	Description
...
Display configuration via `nice\_view\_gem`. WPM fixed range and animation speed can be tuned in the shield conf.
---
Layout Overview
Gallium (Default Layer)
Gallium is an alternative layout designed to minimise same-finger bigrams and lateral stretches. This config uses the columnar-staggered version with X and Q swapped.
```
B  L  D  C  V  |  J  Y  O  U  ?/!
N  R  T  S  G  |  P  H  A  E  I
X  Q  M  W  Z  |  K  F  ,/; ./: -/\_
...
Flashing Firmware
Download
Go to the Actions tab → latest successful build
Download the `firmware` artifact and extract the `.zip`:
...
Destructive BT operations (BT_CLR, BT_CLR_ALL) require a double-tap on Bridge.
---
Battery & Power
...
Edit `config/toucan.keymap`, `config/toucan\_behaviors.dtsi`, or `config/toucan\_combos.dtsi`
Push → GitHub Actions builds automatically
Download the firmware artifact from the Actions tab
The keymap, behaviors, and combos use identical key positions and logic to the Piantor Pro BT config — changes can be ported between boards with minimal adaptation.
Tools:
ZMK Studio — GUI editor (use Studio Unlock on Bridge layer)
keymap-editor — browser-based editor
---
License
Based on ZMK Firmware, MIT License.
The included shield `nice\_view\_gem` is modified from M165437/nice-view-gem, MIT License.
